#!/bin/bash
# =============================================================================
# AURORA-IA / AI-DLC - Project Initialization Script
# =============================================================================
# Initializes a new project workspace with all necessary AURORA-IA structure
...
# 4. Set up all AURORA-IA commands and agents
#
# Usage:
...
EOF
    ...
    if [ -f "$AURORA_ROOT/src/calculator-api/.dependency-cruiser.cjs" ]; then
        cp "$AURORA_ROOT/src/calculator-api/.dependency-cruiser.cjs" "$OUTPUT_DIR/" 2>/dev/null || true
        log_success "dependency-cruiser config copied"
    fi
    if [ -f "$AURORA_ROOT/src/calculator-api/.spectral.yaml" ]; then
        cp "$AURORA_ROOT/src/calculator-api/.spectral.yaml" "$OUTPUT_DIR/" 2>/dev/null || true
...
# Copy AURORA-IA assets
log_step "Copying AURORA-IA assets..."

# Copy memory constitution template
if [ -f "$AURORA_ROOT/memory/constitution.md" ]; then
    cp "$AURORA_ROOT/memory/constitution.md" "$OUTPUT_DIR/memory/constitution.md"
...
if [ -d "$AURORA_ROOT/.github/copilot/agents" ]; then
    cp -r "$AURORA_ROOT/.github/copilot/agents/"* "$OUTPUT_DIR/.github/copilot/agents/" 2>/dev/null || true
    log_success "AI Agents copied (.github/copilot/agents/)"
fi

# Copy .github/commands (Slash Commands)
if [ -d "$AURORA_ROOT/.github/commands" ]; then
    cp -r "$AURORA_ROOT/.github/commands/"* "$OUTPUT_DIR/.github/commands/" 2>/dev/null || true
    log_success "Slash Commands copied (.github/commands/)"
fi

# Copy .github/prompts (Context Prompts)
if [ -d "$AURORA_ROOT/.github/prompts" ]; then
    cp -r "$AURORA_ROOT/.github/prompts/"* "$OUTPUT_DIR/.github/prompts/" 2>/dev/null || true
    log_success "Context Prompts copied (.github/prompts/)"
fi

# Copy .github/workflows (CI/CD Pipelines)
if [ -d "$AURORA_ROOT/.github/workflows" ]; then
    cp -r "$AURORA_ROOT/.github/workflows/"* "$OUTPUT_DIR/.github/workflows/" 2>/dev/null || true
...
if [ -d "$AURORA_ROOT/scripts/bash" ]; then
    for script in "$AURORA_ROOT/scripts/bash/"*.sh; do
...
if [ -d "$AURORA_ROOT/scripts/powershell" ]; then
    for script in "$AURORA_ROOT/scripts/powershell/"*.ps1; do
        script_name=$(basename "$script")
...
Follow these steps to set up your project with AURORA-IA:
...
**Or use the AURORA agent in GitHub Copilot Chat:**
...
│   ├── commands/         # Slash Commands (/aurora.*)
...
|---------|---------|-------|
| \`/aurora.constitution\` | Define project governance & tech stack | Foundation |
...
2. **🎯 Define First Feature** → \`@Aurora Feature\`
3. **📝 Plan Implementation** → \`@Aurora Plan\`
4. **⚡ Start Coding** → \`@Aurora Implement\`

---

*Generated by AURORA-IA Init Script v1.0.0*
EOF
else
    # Brownfield README
    cat > "$OUTPUT_DIR/README.md" << EOF
...
This project has been set up to migrate existing code using the AURORA-IA methodology.
...
\`\`\`
/aurora.feature migrate-[component-name]
\`\`\`
...
4. **📋 Define Features** → \`/aurora.feature migrate-xxx\`
5. **📝 Plan Migration** → \`/aurora.plan\`
6. **⚡ Migrate** → \`/aurora.implement\`
7. **✅ Validate** → \`/aurora.test\`
...
if [ ! -d ".git" ]; then
    git init
    git add .
    git commit -m "🌅 Initial AURORA-IA project setup

- Project type: $([ "$PROJECT_TYPE" = "brown" ] && echo "Brownfield (Migration)" || echo "Greenfield (New Project)")
- Initialized with AURORA-IA + AI-DLC methodology
- Constitution template ready for configuration

Next steps:
1. Edit memory/constitution.md
2. Run /aurora.constitution in Copilot Chat
"
...
echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}                    ✅ AURORA-IA Project Initialized!                   ${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════════════════════════${NC}"
echo ""
...
if [ "$PROJECT_TYPE" = "green" ]; then
    ...
    echo "   4. Start with: /aurora.feature [your-first-feature]"
else
    echo "   2. Open memory/constitution.md"
    echo "   3. Fill in your technology choices"
    echo "   4. Or use: /aurora.constitution in GitHub Copilot Chat"
fi
echo ""
if [ "$PROJECT_TYPE" = "brown" ]; then
...
echo -e "${MAGENTA}🌅 Happy coding with AURORA-IA!${NC}"
echo ""
