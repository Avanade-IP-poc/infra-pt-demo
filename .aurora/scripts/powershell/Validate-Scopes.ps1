...
.DESCRIPTION
    Validates .aurora/scopes/*/scope.yaml files for structure, external source rules,
...
scope_files = sorted(glob.glob('.aurora/scopes/*/scope.yaml'))

if not scope_files:
    err('No scope.yaml files found under .aurora/scopes')
    sys.exit(1)
