...
Our frontend codebase is currently written in JavaScript (ES6+). As the application grows, we're
experiencing increasing issues with:
...
We need to improve code quality, developer productivity, and maintainability of our frontend
codebase as we scale from 50K to 200K lines of code.
...
**Description:** Stop all feature development and rewrite entire codebase to TypeScript in one
sprint.
...
**Why not chosen:** Business cannot afford feature freeze, and risk is too high.

### Option 3: JSDoc Type Annotations

**Description:** Keep JavaScript but add type checking via JSDoc comments.
...
**Why not chosen:** JSDoc is a half-measure that doesn't provide the full benefits of TypeScript.

### Option 4: Status Quo (JavaScript)

**Description:** Continue with JavaScript, invest in better testing instead.
...
**Why not chosen:** Doesn't solve the problem, just works around it.
...
- [ADR-010: Adopt React for UI Framework](./010-react-framework.md) - TypeScript has first-class
  React support
...
Some older dependencies (`legacy-lib`) lack TypeScript definitions. Plan to create and contribute
type definitions to DefinitelyTyped.
