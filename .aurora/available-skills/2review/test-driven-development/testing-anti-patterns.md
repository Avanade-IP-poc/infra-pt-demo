...
public void RendersSidebar()
{
    ...
    result.Should().Contain("<nav");  // Test real behavior
}
...
public class Session
{
    public async Task Destroy()
    {
        ...
        {
            await _workspaceManager.DestroyWorkspace(Id);
        }
        // ... cleanup
    }
}
...
// In tests
public void Dispose() => _session.Destroy().Wait();
```
...
public async Task DisposeAsync()
    => await SessionTestHelpers.CleanupSession(_session);
```
...
public async Task DetectsDuplicateServer()
{
    ...
    mockToolCatalog
        .Setup(t => t.DiscoverAndCacheTools())
        .Returns(Task.CompletedTask);

    await AddServer(config, mockToolCatalog.Object);
    await AddServer(config, mockToolCatalog.Object); // Should throw - but won't!
}
```
...
public async Task DetectsDuplicateServer()
{
    ...
    mockServerManager.Setup(m => m.StartServer(It.IsAny<ServerConfig>()))
        .Returns(Task.CompletedTask);

    await AddServer(config, mockServerManager.Object); // Config written
    var act = async () => await AddServer(config, mockServerManager.Object);
    await act.Should().ThrowAsync<DuplicateServerException>(); // Duplicate detected ✓
}
```
...
**If you're testing mock behavior, you violated TDD** - you added mocks without watching test fail
