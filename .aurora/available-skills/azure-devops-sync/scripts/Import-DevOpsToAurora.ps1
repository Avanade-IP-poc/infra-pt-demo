...
.DESCRIPTION
    ...
    and generates the corresponding AURORA spec structure in specs/ folder.
    
    Useful for retroactively converting existing DevOps backlog to AURORA methodology.

.PARAMETER WorkItemId
    The ID of the Feature work item to import

.PARAMETER OutputPath
    Target path for the AURORA spec (e.g., "specs/002-imported-feature")
...
.EXAMPLE
    .\Import-DevOpsToAurora.ps1 -WorkItemId 12345 -OutputPath "specs/001-time-tracking"
    Import Feature 12345 with all children

.EXAMPLE
    .\Import-DevOpsToAurora.ps1 -WorkItemId 12345 -OutputPath "specs/001-time-tracking" -IncludeChildren:$false
    ...
    $metadata = @{
        ...
        azureDevOps     = @{
            ...
            userStories           = @($UserStories | ForEach-Object {
                @{
                    workItemId    = $_.id
                    auroraStoryId = "US-$((1 + $UserStories.IndexOf($_)).ToString('000'))"
            ...
            tasks                 = @($Tasks | ForEach-Object {
                @{
                    workItemId   = $_.id
                    auroraBoltId = "$featureId-$((1 + $Tasks.IndexOf($_)).ToString('000'))"
                    title        = $_.fields.'System.Title'
                    state        = $_.fields.'System.State'
                }
            })
...
if ($workItemType -ne "Feature") {
    ...
    Write-StatusMessage "Only Feature work items can be imported as AURORA specs" -Type Warning
    exit 1
}
...
Write-Host "  2. Refine with AURORA agents:" -ForegroundColor White
Write-Host "     @Aurora Feature review $OutputPath" -ForegroundColor Cyan
Write-Host "     @Aurora Plan verify implementation plan" -ForegroundColor Cyan
