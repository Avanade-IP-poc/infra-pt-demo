...
> Complete workflow example for starting a new project with AURORA-IA-DLC.
...
For each Bolt, invoke `@Aurora Implement`:
...
- `@Aurora Testing` — Verify coverage meets thresholds
- `@Aurora Review` — Code review against constitution

### 8. Release

When all Bolts complete:

- `@Aurora Release` — Package, changelog, tag, deploy
