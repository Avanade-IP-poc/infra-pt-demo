...
| `mcp_github_get_issue`         | Fetch issue details    |
...
1. **Gather context**: Get repo info, existing labels, milestones if needed
1. **Structure content**: Use appropriate template from
   [references/templates.md](references/templates.md)
1. **Execute**: Call the appropriate MCP tool
1. **Confirm**: Report the issue URL to user
...
```text
labels: ["iteration", "priority", "documentation", ...]
...
Always use the templates in [references/templates.md](references/templates.md). Choose based on
issue type:
...
Use `mcp_github_update_issue` with:
...
**Action**: Call `mcp_github_create_issue` with:
...
- Any dependencies must be documented as a sub-issue
