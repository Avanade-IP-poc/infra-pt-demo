...
echo "🏗️  AURORA Project Structure Generator"
echo "====================================="
...
if [[ $TECH_STACK == *".NET"* ]]; then
    ...
    cd src/backend
    dotnet new sln -n Aurora
    dotnet new webapi -n Aurora.Api
    dotnet sln add Aurora.Api
...
if [[ -f "src/backend/package.json" ]]; then
    ...
    cd ../..
elif [[ -f "src/backend/Aurora.sln" ]]; then
...
> AURORA-powered project with intelligent development lifecycle
...
# Backend
$(if [[ $TECH_STACK == *".NET"* ]]; then
echo "cd src/backend/Aurora.Api && dotnet run"
fi)
$(if [[ $TECH_STACK == *"Python"* ]]; then
...
*Generated by AURORA-IA-DLC v2.1.0*
EOF
