...
### Base URL
```
Production: https://api.aurora.company.com/v1
Staging: https://staging-api.aurora.company.com/v1
Development: https://dev-api.aurora.company.com/v1
```
...
```json
{
  "type": "https://api.aurora.company.com/problems/validation-error",
...
```bash
curl -X GET "https://api.aurora.company.com/v1/{resources}?status=active" \
...
```bash
# Create resource
curl -X POST "https://api.aurora.company.com/v1/{resources}" \
  -H "Authorization: Bearer {JWT_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Test Resource",
...
# Update resource
curl -X PUT "https://api.aurora.company.com/v1/{resources}/{id}" \
  -H "Authorization: Bearer {JWT_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
...
const client = new AuroraAPI({
  baseUrl: 'https://api.aurora.company.com/v1',
...
var client = new AuroraApiClient("https://api.aurora.company.com/v1", "your-jwt-token");
...
```json
{
  "base_url": "https://staging-api.aurora.company.com/v1",
...
### Support Channels
- **Documentation:** [https://docs.aurora.company.com](https://docs.aurora.company.com)
